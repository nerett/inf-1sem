...
const size_t START_CAPACITY = 10000000; //8
...
void StackCtor(struct Stack *some_stack,
               user_dump * user_type_dump_function,
               err_code * error_variable);
void StackDtor(struct Stack *some_stack, err_code * error_variable);

void stack_push(struct Stack *some_stack, stk_element_t value,
                err_code * error_variable);
stk_element_t stack_pop(struct Stack *some_stack,
                        err_code * error_variable);
static void stack_resize(struct Stack *some_stack,
                         err_code * error_variable);
...
static double calc_smoothing_downsize_coeff(const struct Stack
                                            *some_stack);

static void upsize_stack(struct Stack *some_stack,
                         err_code * error_variable);
static void downsize_stack(struct Stack *some_stack,
                           err_code * error_variable);
static void reallocate_stack(struct Stack *some_stack,
                             err_code * error_variable);

static bool validate_stack(struct Stack *some_stack, err_code * error_variable);        //< Returns true, if stack is OK
void stack_dump(struct Stack *some_stack, err_code stack_error,
                const char *error_filename, const char *error_function,
                int error_line, err_code * error_variable);
void remake_log(err_code * error_variable);

static void set_data_canaries(struct Stack *some_stack);
static unsigned long long struct_canary_x_ptr(unsigned long long
                                              *canary_ptr);
static stk_element_t data_canary_x_ptr(stk_element_t * stack_data,
                                       int offset);
...
static void set_stack_data_hash(struct Stack *some_stack,
                                err_code * error_variable);
...
void stack_push(struct Stack *some_stack, stk_element_t value, err_code * error_variable)       //есть какая-то ошибка памяти
{
    ...
    /*
       if (some_stack->is_initialized != true) {
       return;
       }
     */
...
#ifndef NDEBUG
    some_stack->left_struct_canary =
    struct_canary_x_ptr(&some_stack->left_struct_canary);
    some_stack->right_struct_canary =
    struct_canary_x_ptr(&some_stack->right_struct_canary);
...
static bool validate_stack(struct Stack *some_stack,
                           err_code * error_variable)
{
    return 0;                   //костыльное ускорение
                ...
                int error_line, err_code * error_variable)
{
    ...
    if (stack_error == OK) {
        fprintf(logfile,
                "<font color=\"green\">[OK] at function %s at %s:%d</font>\n",
                error_function, error_filename, error_line);
    } else {
        fprintf(logfile,
                "<font color=\"red  \">[ERROR] stack_error_code = %d at function %s at %s:%d</font>\n",
                stack_error, error_function, error_filename, error_line);
        fprintf(logfile, "\tstruct Stack\n");
        fprintf(logfile, "\t{\n");
        fprintf(logfile, "/tmax_capacity = %d\n",
        ...
        if ((some_stack->user_type_dump_function != NULL)
            && some_stack->N_element > -1) {
            char *user_type_dump_string =
            some_stack->user_type_dump_function(some_stack->data,
                                                    some_stack->N_element);
            if (user_type_dump_string == NULL) {
                error_output(error_variable,
                             INVALID__USER_TYPE_DUMP_STRING_PTR);
                return;
            }

            fprintf(logfile, "/tdata = { ");
...
static void set_data_canaries(struct Stack *some_stack)
{
    ...
    *(some_stack->data + some_stack->max_capacity + DATA_PTR_OFFSET) =
    data_canary_x_ptr(some_stack->data,
                          some_stack->max_capacity + DATA_PTR_OFFSET);
}
...
static stk_element_t data_canary_x_ptr(stk_element_t * stack_data,
                                       int offset)
{
    return (stk_element_t) (CANARY ^
                            (unsigned long long) (stack_data + offset));
}
...
{
    void *realloc_buffer =
    (stk_element_t *) realloc(some_stack->data - DATA_PTR_OFFSET,
                                  sizeof(stk_element_t) *
                                  (some_stack->max_capacity + 1 +
                                   N_CANARIES));

    if (realloc_buffer != NULL) {
        some_stack->data =
        (stk_element_t *) realloc_buffer + DATA_PTR_OFFSET;
    } else {
...
void set_stack_data_hash(struct Stack *some_stack, err_code * error_variable)   //не проверяет валидность стека в силу логики использования
{
    ...
    int hash_key_buffer =
    xor_hash(some_stack->data, some_stack->N_element);
...
int main()
{
    ...
    for (int increment = 0; increment < length; increment++) {
        ...
        } else                  //если закрывающая скобка
        {
            ...
            if ((pop_bracket - symbol) > 1)     //если скобочки разные
            {
                printf("NO\n");
                StackDtor(&stack, &error_variable);
                return 0;
